...
The optional Reddit publisher uses the [official OAuth refresh flow](https://github.com/reddit-archive/reddit/wiki/OAuth2) and [Reddit API](https://www.reddit.com/dev/api/) with separate `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET`, `REDDIT_REFRESH_TOKEN`, `REDDIT_USER_AGENT` and `REDDIT_ALLOWED_SUBREDDITS` environment variables. `REDDIT_API_APPROVED=true` must be set explicitly. Refresh-token scopes must include `identity`, `read`, and `submit`; no X browser session is used. The target must belong to an approved community, match its exact fresh source hash, and either mention the configured Reddit account or be a verified direct reply to its content. Account identity, opt-outs, archived/locked/removed targets and exact parent/text receipts are checked. Confirmation independently reads the created comment. Reconciliation accepts only a unique exact account/parent/text receipt in the bounded original-attempt time window; `publicationAttemptedAt` is supplied by the controller. Missing or ambiguous evidence remains unknown and never authorizes another POST. Reddit ingestion is not implemented; use controlled manual intake with the original URL.
