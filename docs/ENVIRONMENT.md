...
For conditional Reddit publication, configure worker-only `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET`, `REDDIT_REFRESH_TOKEN`, `REDDIT_USER_AGENT`, and comma-separated `REDDIT_ALLOWED_SUBREDDITS`. Keep worker `REDDIT_API_APPROVED=false` until official account/community access is authorized. OAuth scopes must include `identity`, `read`, and `submit`; do not put these credentials in Convex or Next.js. The worker verifies the account, fresh target, direct contact intent, exact text, and independent receipt. Reddit ingestion is not implemented; use controlled manual intake with the original URL. See [worker setup](../workers/README.md) for the contract and reconciliation rules.
