...
export function Dashboard({tab="cases",caseId}:{tab?:Tab;caseId?:string}) {
  ...
  return <div className="mend-app"><MendHeader tab={tab}/>
    {snapshot&&<div className="workspace-toolbar"><WorkspaceStrip snapshot={snapshot} connection={connection} lastSyncedAt={lastSyncedAt}/>{snapshot.mode==="demo"?<aside aria-label="Demo mode" className="demo-identity"><span className="demo-data-badge" title="Fictional sample records and local workflow playback. No live customer activity.">Demo data</span><label>View as<select aria-label="Demo identity" value={snapshot.actor.roles[0]} disabled={busy} onChange={event=>void act("demo_role",{role:event.target.value},"Workspace role changed.")}><option value="engineer">Engineer</option><option value="marketer">Marketer</option><option value="admin">Admin</option></select></label></aside>:<span className="operator-name">{snapshot.actor.name}</span>}</div>}
