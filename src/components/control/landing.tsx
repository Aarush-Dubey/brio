  ...
  return <div className="mend-app"><MendHeader /><main id="main-content" className="landing">
    ...
    <section className="landing-story" id="walkthrough"><div className="story-grid">
      ...
      <div className="story-visual"><div className="story-example" key={chapter}>
        {chapter === 0 && <div className="tweet-example"><div className="tweet-meta"><span className="avatar"/><div><strong>Weather customer</strong><span>@weather_customer · X</span></div><span className="mono">Fixture</span></div><p>@drizzleapp it says 20°C, but switching to Fahrenheit shows 20°F. That should be 68°F. Can you check the conversion?</p><small>Original wording retained · simulated example</small></div>}
        ...
        {chapter === 5 && <><div className="eyebrow">CLOSE THE LOOP</div><h3 className="example-heading">Fixed and notified are separate facts.</h3><div className="example-line">Production behavior verified <span>required</span></div><div className="example-line">Exact reply approved <span>required</span></div><div className="example-line">Publication receipt confirmed <span>required</span></div><Link className="text-link" href="/cases">Follow the simulated run on the board →</Link></>}
      </div></div>
    </div></section>
