...
import { caseKind, formatTime, reportCount, shortId, stageFor, STAGES } from "./mend";
...
export function CasesView({ snapshot, act, busy, connection = "connecting", recentTransitions = [] }: { snapshot: Snapshot; act: Action; busy: boolean; connection?: string; recentTransitions?: Transition[] }) {
  ...
  const events = run?.events.map(event => ({id:event.id,at:event.at,title:event.title,detail:event.detail,caseId:run.caseId})) ?? recentTransitions.map(event => ({id:`${event.caseId}:${event.at}`,at:event.at,title:event.title,detail:`${humanize(event.from)} → ${humanize(event.to)}`,caseId:event.caseId}));
  return <>
    ...
    {snapshot.mode === "demo" && <section className="demo-run-bar" aria-label="Live demo controls">
      <div><span className="eyebrow accent">LOCAL DEMO · SIMULATED INTEGRATIONS</span><strong>{run?.stopReason ? `Demo stopped: ${humanize(run.stopReason)}` : run?.status === "running" ? run.stepLabel : run?.status === "paused" ? "Demo paused" : run?.status === "completed" ? "Demo complete. Every step is recorded." : "Watch a complaint move through the board."}</strong><p>No live posts, paid model calls, production releases, or real Slack decisions.</p></div>
      <div className="demo-run-actions">
        {!run || run.status === "completed" ? <button className="accent-button" disabled={busy} onClick={() => void act("demo_start", {}, "Simulated workflow started.")}>▶ Run demo</button> : run.status === "running" ? <button disabled={busy} onClick={() => void act("demo_pause", {runId:run.runId}, "Demo paused.")}>Ⅱ Pause</button> : <button className="accent-button" disabled={busy} onClick={() => void act("demo_resume", {runId:run.runId}, "Demo resumed.")}>▶ Resume</button>}
        {run && <button className="secondary" disabled={busy} onClick={() => void act("demo_restart", {runId:run.runId}, "A fresh simulated run started.")}>↻ Restart</button>}
    ...
    <section className="activity-panel" aria-label="Live event feed"><div className="activity-header"><h2><i className="status-dot"/>Live activity</h2><span className="mono">{snapshot.mode === "demo" ? "simulated events" : "recorded transitions"}</span></div>{events.length ? [...events].sort((a,b)=>b.at-a.at).slice(0,7).map(event=><Link href={`/cases/${event.caseId}`} className="activity-event" key={event.id}><time className="mono">{formatTime(event.at)}</time><strong>{event.title}</strong><span>{event.detail}</span><span>↗</span></Link>) : <p className="activity-empty">{snapshot.mode === "demo" ? "Run the demo to watch each recorded transition appear here." : "Case transitions will appear as work progresses."}</p>}</section>
  </>;
}
